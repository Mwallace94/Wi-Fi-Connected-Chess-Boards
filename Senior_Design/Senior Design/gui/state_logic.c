...
int isLegal(transition t) {
	...
	if(t.piece == WPAWN) {
		...
		if(ydist == 1 && !xdis && !state[t.toRow][t.toCol]) return 1;
		else if(t.fromRow == 1 && ydist == 2 && !state[t.toRow][t.toCol] && !state[t.toRow-1][t.toCol]) return 1;
	...
	if(t.piece == BPAWN) {
		...
		//should always be true for move only
		if(ydist == 1 && !xdis && !state[t.toRow][t.toCol]) return 1;
		else if(t.fromRow == 6 && ydist == 2 && !state[t.toRow][t.toCol] && !state[t.toRow+1][t.toCol]) return 1;
		//add taking piece here
		else return 0;
	}
	...
	if(t.piece == WKNIGHT || t.piece == BKNIGHT) {
		//detects move only
		if(ydis && xdis && dis == 3 && !state[t.toRow][t.toCol]) {
			return 1;
		} else return 0;
	}
	...
	if(t.piece == BBISHOP || t.piece == WBISHOP) { 
		...
		if(ydis != xdis) {
			...
			for(i = 1; i <= ydis; i++) {
				if(state[t.fromRow+i][t.fromCol+i]) return 0;
			}
		} else if(t.fromRow > t.toRow && t.fromCol < t.toCol) {
			for(i = 1; i <= ydis; i++) {
				if(state[t.fromRow-i][t.fromCol+i]) return 0;
	...
	if(t.piece == BKING || t.piece == WKING) {
		if(dis == 1 && !state[t.toRow][t.toCol]) return 1;
		else return 0;
	}
	return 0;
}
