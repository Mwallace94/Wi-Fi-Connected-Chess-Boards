...
int checkKingSurrounding(transition t){
  ...
  printf("\nKing cannot be moved to prevent its death. What about castling?\n");

 if(tnew.piece == BKING){
    if(rightRookBMovedOnce != 1 && kingBMovedOnce != 1){
      tnew.toRow =  0;
      tnew.toCol =  8;
      if(isLegal(tnew)){
        teststate[tnew.toRow][tnew.toCol] = (char) (theircolor +6);
        ...
        for(l = 0; l < 8; l++){
          for(m = 2; m < 10; m++){
            if(teststate[l][m] > (char)(yourcolor) && state[l][m] <= (char)(yourcolor+5)){
                ttest.fromRow = l;
                ttest.fromCol = m;
                ttest.toRow = tnew.toRow;
                ttest.toCol = tnew.toCol;
            ...
            if(safeAroundKing == 1){
               printf("\nKing can be castled to prevent its death\n");
               return 1;
            }
           initTestState();
          }
      }if(leftRookBMovedOnce != 1 && kingBMovedOnce != 1){
        tnew.toRow = 0;
        tnew.toCol = 4;
        if(isLegal(tnew)){
          teststate[tnew.toRow][tnew.toCol] = (char) (theircolor +6);
    ...
    }else if(tnew.piece == WKING){
      if(rightRookWMovedOnce != 1 && kingWMovedOnce != 1){
      tnew.toRow =  7;
      tnew.toCol =  8;
      if(isLegal(tnew)){
        teststate[tnew.toRow][tnew.toCol] = (char) (theircolor +6);
  ...
  printf("\nKing cannot be castled to prevent its death\n");
  return 0;
}
